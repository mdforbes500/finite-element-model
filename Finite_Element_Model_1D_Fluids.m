...
Q = zeros(1,2*N+1);
Q(1) = pressure_diff/(xB-xA);
Q(2*N+1) = -pressure_diff/(xA-xB);

% Elemental force vector 
F_e = cell(1,2*N+1);
for i = 1:2*N+1
    F_e{i} = pressure_diff*h/3.*[1;4;1];
end
...
for i = 1:RES+1
    ...
    %Plotting data
    func = xB^2*pressure_diff/(2*a).*(1.-(omega.^2./xB^2));
    figure
    hold on
